...
pub fn parallel_perft(game: &Game, depth: usize) -> Fallible<usize> {
    fn go(game: &Game, depth: usize, m: &GameMove<Coord>) -> usize {
        let mut copy_game = (*game).clone();
        copy_game.apply_move(m).unwrap();
        parallel_perft(&copy_game, depth - 1).unwrap()
    }
    ...
    if depth == 1 {
        ...
        Ok(moves.par_iter().map(|m| go(game, depth, m)).sum())
    }
}
...
/// mutations.
pub fn perft(game: &mut Game, depth: usize) -> Fallible<usize> {
    let moves = game.generate_moves();
    if depth == 1 {
        return Ok(moves.len());
    }
    ...
    let mut result = 0;
    for m in moves {
        let fight_result = game.apply_move(&m)?;
